...
# PODNAME: greedy-motif-search-pseudo.pl
# ABSTRACT: Greedy Motif Search With Pseudocounts
...
# Default options
my $input_file = 'greedy-motif-search-pseudo-sample-input.txt';
my ( $debug, $help, $man );
...
printf "%s\n", join "\n", greedy_motif_search_pseudo( \@dna, $k, $t );

sub greedy_motif_search_pseudo {
    ...
    foreach my $i ( 0 .. ( length $dna->[0] ) - $k ) {
        ...
        foreach my $j ( 1 .. $t - 1 ) {
            my $profile = profile_pseudo( \@motifs, $k, $j );
...
This script implements Greedy Motif Search with pseudocounts.
...
B<GreedyMotifSearch>(I<Dna>,I<k>,I<t>) with pseudocounts. If at any step you
find more than one I<Profile>-most probable I<k>-mer in a given string, use the
one occurring first.

=head1 EXAMPLES

    perl greedy-motif-search-pseudo.pl

    perl greedy-motif-search-pseudo.pl \
        --input_file greedy-motif-search-pseudo-extra-input.txt

    diff <(perl greedy-motif-search-pseudo.pl) \
        greedy-motif-search-pseudo-sample-output.txt

    diff \
        <(perl greedy-motif-search-pseudo.pl \
            --input_file greedy-motif-search-pseudo-extra-input.txt) \
       greedy-motif-search-pseudo-extra-output.txt

    perl greedy-motif-search-pseudo.pl --input_file dataset_160_9.txt \
        > dataset_160_9_output.txt
